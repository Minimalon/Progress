...
function check_error_UTM {
    ...
    if [ $checkError == 1 ]; then
        ...
        x=$((x + 1))
        if  (( x >= 3)); then
            printf "`date +"%H:%M %d/%m/%Y"`\t$fsrar\t`uname -n | cut -d '-' -f2,3`\UTM_ERROR\tERROR - Не поднимается утм" >> /linuxcash/net/server/server/resendPivo.log
            exit
        fi
    fi
}
...
check_current_ReplyNaTTN 18082
check_accepted_TTN 18082
...
countRegInfo=`links -dump http://localhost:18082/opt/out | grep -c FORM2REGINFO` ; echo Накладных на УТМ: $countRegInfo
...
if [[ $countRegInfo < $countTTN ]]; then
   ...
   echo  Нету на УТМ $count
	 ...
	 curl -F "xml_file=@QueryResendDoc.xml" http://localhost:18082/opt/in/QueryResendDoc 2>/dev/null
