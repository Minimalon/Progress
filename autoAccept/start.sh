...
# Принимает $1=shipperID $2=ttnNumber $3=Date $4=Дней до автоприёма
function notification {
    ...
    notification_path="/root/notifications"
    shipper_name=$(grep -A1 -B4 "$shipperID" /linuxcash/net/server/server/All/Artur/flags/shippers.json | jq .name | tr -d '"\\')
    printf 'Принята пивная накладная %s %s %s' "$ttnNumber" "$shipper_name" "$date" >> $notification_path/autoAccept_"$ttnNumber"
        ...
        if (( $countID >= 1 )); then
            ...
            if (( "$(links -source http://localhost:"$port"/opt/out | grep "$id" | grep -c 'ReplyClient_v2')" >= 1 )); then # Если Accepted ReplyClient_v2
                ...
                if [[ $(grep -c "$ClientRegId" /linuxcash/net/server/server/whitelist_autoaccept.txt) == 0 ]]; then
                 if [[ $(grep -c "$ClientRegId" /linuxcash/net/server/server/BADwhitelist_autoaccept.txt) == 0 ]]; then
...
function accepted_TTN () {
    ...
    ttnNumber=($(links -source "$ReplyAdress" | sed "s/> */>\n/g" | grep "ttnNumber" | awk -F "<|>" {'print $1'})) # Даты накладных
    ...
    for date in "${dateTTN[@]}"; do # Перебираем все даты ТТНок из ReplyNaTTN
      ...
      if [[ $bad_fsrar -gt 0 && $port == "8082" && inn == "11" ]]; then # Если плохой поставщик, порт 8082 и длина ИНН как у ООО, то не принимаем
        echo "Плохая фсрар ---> server/BADwhitelist_autoaccept.txt"
		continue
      fi

        if [[ $whitelist_fsrar -gt 0 ]]; then # Если есть поставщик в белом списке
          if (( $date <= $oldDate )); then # Если дата меньше (текущая дата без деффиса < максимальный возраст накладной в днях)
              ...
              if (( $(grep -c "${TTNs[$count]}" acceptedTTN) >= 1 )); then # Если есть совпадение в списке принятых тикетов, то ничего не делает, иначе принимаем
                  ...
                  notification "${shipper_fsrar[$count]}"  "${ttnNumber[$count]}"  "${printdateTTN[$count]}" "$1"
                  if (( $yearTTN >= "2022" )); then # WB_4
                      cd /root/ArturAuto/autoAccept/WayBillAct_v4 || exit
                      ...
                      echo "$(date +"%H:%M %d/%m/%Y") Принимаю накладную ${TTNs[$count]}" >> /root/ArturAuto/autoAccept/acceptedTTN
                      printf "\n-------------------------------\n"
                  ...
                  if (( $yearTTN == "2021" && "$monthTTN" <= "05" )); then # WB_3
                      cd /root/ArturAuto/autoAccept/WayBillAct_v3 || exit
                      sed -e "s/ID_t/$fsrar/g" accepted.xml.prepare >accepted.xml.prepare.1
                      sed -e "s/TTNREGID/${TTNs[$count]}/g" accepted.xml.prepare.1 > accepted.xml.prepare.2
                      sed -e "s/nowdate/$nowdate/g" accepted.xml.prepare.2 > accepted.xml
                  ...
                  if (( $yearTTN == "2018" && "$monthTTN" == "03" && "$dayTTN" <= "14" )); then # WB_2
                      cd /root/ArturAuto/autoAccept/WayBillAct_v2 || exit
                      sed -e "s/ID_t/$fsrar/g" accepted.xml.prepare >accepted.xml.prepare.1
                      sed -e "s/TTNREGID/${TTNs[$count]}/g" accepted.xml.prepare.1 > accepted.xml.prepare.2
                      sed -e "s/nowdate/$nowdate/g" accepted.xml.prepare.2 > accepted.xml
                  ...
                  if (( $yearTTN == "2017" && "$monthTTN" <= "06" )); then # WB_1
                      cd /root/ArturAuto/autoAccept/WayBillAct || exit
                      sed -e "s/ID_t/$fsrar/g" accepted.xml.prepare >accepted.xml.prepare.1
                      sed -e "s/TTNREGID/${TTNs[$count]}/g" accepted.xml.prepare.1 > accepted.xml.prepare.2
                      sed -e "s/nowdate/$nowdate/g" accepted.xml.prepare.2 > accepted.xml
...
function main {
    port=$1
    cd /root/ArturAuto/autoAccept || exit
