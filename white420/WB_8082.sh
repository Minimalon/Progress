...
whitelsts=($(links -source http://localhost:8082/opt/out | grep WayBill_v4 | awk -F "<|>" {'print $3'}))
tempCount=1
...
if [ "$line" -eq 1 ]; then
	...
	for line in $whitelsts; do
	  ...
	  links -source http://localhost:8082/opt/out | awk -F "<|>" {'print $3'} | grep  FORM2REGINFO >> FORM2REGINFO.txt
	  ...
	  for reg in $regs; do
	    ...
	    if [ "$number" == "$numberreg" ]; then
	      ...
	      curl -F "xml_file=@accepted.xml" http://localhost:8082/opt/in/WayBillAct_v4 > /dev/null 2>&1
  ...
  rm "$nameFile"  2>/dev/null
  links -dump http://localhost:8082/opt/out 2>/dev/null | grep  FORM2REGINFO >> FORM2REGINFO.txt
  regs=($(cat FORM2REGINFO.txt | awk {'print $1'}))
	 for reg in "${regs[@]}"; do
    wget "$reg" 2>/dev/null
    friTTN=$(links -source "$reg" | grep "<wbr:WBRegId>" | cut -d '>' -f2 | cut -d '<' -f1)
