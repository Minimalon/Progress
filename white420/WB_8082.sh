...
if [ $line -eq 1 ]; then
	...
	for line in $whitelsts; do
	  ...
	  echo $number
	  rm $nameFile
	  links -dump http://localhost:8082/opt/out | grep  FORM2REGINFO >> FORM2REGINFO.txt
	  ...
	  for reg in $regs; do
	    ...
	    if [ "$number" == "$numberreg" ]; then
	      ...
	      curl -F "xml_file=@accepted.xml" http://localhost:8082/opt/in/WayBillAct_v4 2>/dev/null
	      curl -X DELETE $line
	      curl -X DELETE $reg
