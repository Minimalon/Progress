...
links -dump http://localhost:8082/opt/out | grep -v 'refused' > info.txt
fileName=/root/white420/info.txt
links -dump http://localhost:8082/opt/out | grep ReplyNATTN  | awk {'print $1'} >> ReplyNATTN.txt
...
if [ "$ReplyDate" == "$nowdate" ]; then
	...
	echo "$ReplyDate и $nowdate"
	rm ReplyNATTN.txt
	curl -F "xml_file=@QueryNATTN.xml" http://localhost:8082/opt/in/QueryNATTN
fi
...
if [ -f $fileName ] && [ -s $fileName ]; then
	...
	while true
	do
	    aaa=`links -dump  http://localhost:8082/opt/out | grep ReplyNATTN | awk {'print $1'} `
	    if [ "$aaa" == "" ]; then
...
do
	...
	if (( $yearTTN == "2021" && 10#$monthTTN <= "05" )); then # WB_3
		...
		sed -e "s/TTNREGID/$ttn/g" accepted.xml.prepare.1 > accepted.xml.prepare.2
		sed -e "s/nowdate/$nowdate/g" accepted.xml.prepare.2 > accepted.xml
		curl -F "xml_file=@accepted.xml" http://localhost:8082/opt/in/WayBillAct_v3
		printf "\n-------------------------------\n"
	fi
	...
	if (( $yearTTN == "2018" && 10#$monthTTN == "03" && 10#$dayTTN <= "14" )); then # WB_2
		...
		sed -e "s/TTNREGID/$ttn/g" accepted.xml.prepare.1 > accepted.xml.prepare.2
		sed -e "s/nowdate/$nowdate/g" accepted.xml.prepare.2 > accepted.xml
		curl -F "xml_file=@accepted.xml" http://localhost:8082/opt/in/WayBillAct_v2
		printf "\n-------------------------------\n"
	fi
	...
	if (( $yearTTN == "2017" && 10#$monthTTN <= "06" )); then # WB_1
		...
		sed -e "s/TTNREGID/$ttn/g" accepted.xml.prepare.1 > accepted.xml.prepare.2
		sed -e "s/nowdate/$nowdate/g" accepted.xml.prepare.2 > accepted.xml
		curl -F "xml_file=@accepted.xml" http://localhost:8082/opt/in/WayBillAct
		printf "\n-------------------------------\n"
	fi
